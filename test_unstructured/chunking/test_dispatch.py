...
class Describe_add_chunking_strategy:
    ...
    def but_it_skips_dispatches_when_no_chunking_strategy_is_specified(self):
        decorated_partitioner = add_chunking_strategy(partition_this)
...
class Describe_chunk:
    ...
    def it_dispatches_to_the_chunker_registered_for_the_chunking_strategy(self):
        ...
        chunks = chunk([], "by_something_else", **kwargs)

        assert chunks == [
            CompositeElement("chunked 0 elements with `(max_characters=750, whizbang=None)`")
        ]
